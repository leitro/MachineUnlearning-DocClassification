...
from dataset_unlearn_aug_randGen_forget_solo import loadData_full, loadData_randGen, NUM_CLASS, UNCLASS
...
def train(prefix, weights=None):
    if FIX_SEED:
        set_seed(42)
    data = loadData_randGen()
    ...
    elif prefix == 'randomimage':
        data_cat_randomimage = torch.utils.data.ConcatDataset([data['train']['retain'], data['train']['forget']])
        dataloader_cat_randomimage = torch.utils.data.DataLoader(data_cat_randomimage, batch_size=BATCH_SIZE, shuffle=True, num_workers=NUM_THREAD, collate_fn=collate_batch)
        dataloader_cur = dataloader_cat_randomimage
...
if __name__ == '__main__':
    ...
    for tb in ['gen']:
        key = f'{tb}'
        result[key] = []
        ret = train('randomlabel', 'rvl-41.model.87.93.scratch')
        result[key].append(ret)
        ret = train('randomimage', 'rvl-41.model.87.93.scratch')
        result[key].append(ret)

    np.save(f'randGen_result_iter300.npy', result)
